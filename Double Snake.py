# -*- coding: utf-8 -*-
"""
Created on Wed Dec  5 21:28:41 2018
...
def display_score_2(score):
    screen_text = font.render(score, True, YELLOW)
    game_display.blit(screen_text, [50, 70])
...
def game_loop():
    ...
    while running:
        while menu:
            ...
            texto('Double Snake', CYAN, 80, 260, 30)
            pygame.draw.rect(game_display, WHITE, [330, 320, 135, 30])
            texto('Jogar (J)', BLACK, 30, 335, 325)
            pygame.draw.rect(game_display, WHITE, [330, 365, 135, 30])
            texto('Opções (O)', BLACK, 30, 335, 370)
            pygame.draw.rect(game_display, WHITE, [330, 410, 135, 30])
            texto('Sair (S)', BLACK, 30, 335, 415)
        ...
        while game_over:        
            if score_2 > score:  
                ...
                texto(' O Jogador 2 é o:', DARK_CYAN, 80,180, 30)
                texto('VENCEDOR', CYAN, 85, 230, 120)
                texto('Score: {0} x {1}'.format(score_2, score), WHITE, 55, 260, 360)
                texto('Carregando...', WHITE, 35, 600,550)
                pygame.display.update()
                time.sleep(5)
                game_loop()
            ...
            if score > score_2:  
                game_display.fill(background_color)
                texto(' O Jogador 1 é o:', DARK_CYAN, 80,180, 30)
                texto('VENCEDOR', CYAN, 85, 230, 120)
                texto('Score: {0} x {1}'.format(score, score_2), WHITE, 55, 260, 360)
                texto('Carregando...', WHITE, 35, 600,550)
                pygame.display.update()
                time.sleep(5)
                game_loop()
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_a:
                    lead_x_2_change = -block_size
                    lead_y_2_change = 0
                elif event.key == pygame.K_d:
                    lead_x_2_change = block_size
                    lead_y_2_change = 0
                if  event.key == pygame.K_w:
                    lead_y_2_change = -block_size
                    lead_x_2_change = 0
        ...
        if lead_x >= display_width or lead_x < 0 or lead_y >= display_height or lead_y < 0:
            game_over = True
        if lead_x_2 >= display_width or lead_x_2 < 0 or lead_y_2 >= display_height or lead_y_2 < 0:
            game_over = True

        lead_x += lead_x_change
        lead_y += lead_y_change
        lead_x_2 += lead_x_2_change
        ...
        snake(block_size, block_size, snake_list_2)
        display_score('Score 1: ' + str(score))
        display_score_2('Score 2: ' + str(score_2))
        pygame.display.update()
        ...
        for index in range(len(left_bombs_x) - 1):
            if get_collision(lead_x, lead_y, left_bombs_x[index], left_bombs_y[index], block_size, block_size):
                game_over = True
        for index in range(len(up_bombs_x) - 1):
            if get_collision(lead_x_2, lead_y_2, up_bombs_x[index], up_bombs_y[index], block_size, block_size):
                game_over = True
        for index in range(len(left_bombs_x) - 1):
            if get_collision(lead_x_2, lead_y_2, left_bombs_x[index], left_bombs_y[index], block_size, block_size):
                game_over = True
        #Player collision
        
        if get_collision(lead_x_2, lead_y_2, lead_x, lead_y, block_size, block_size):
                game_over = True
        ...
        if get_collision(rand_apple_x, rand_apple_y, lead_x, lead_y, block_size, block_size):
            ...
            background_color = BLACK
        if get_collision(rand_apple_x, rand_apple_y, lead_x_2, lead_y_2, block_size, block_size):
            rand_apple_x = round(random.randrange(0, display_width - block_size) / block_size) * block_size
            rand_apple_y = round(random.randrange(0, display_height - block_size) / block_size) * block_size
            snake_length_2 += 1
            score_2 += score_add_2
            display_score_2(str(score_2))
            background_color = BLACK
        # POWER-UP COLLISION
        ...
        if get_collision(powerup_x, powerup_y, lead_x_2, lead_y_2, block_size * 2, block_size * 2):
            score_2 += 50
            powerup_x = 1000
            powerup_y = 1000
